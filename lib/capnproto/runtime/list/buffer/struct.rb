...
class CapnProto::StructList < CapnProto::BufferList
  ...
  private def get(ix)
    ...
    when 2, 3, 4, 5
      ...
      element_class.new(@data.offset_position(data_offset), @element_size, CapnProto::Reference::EMPTY, 0)

      # Pointer type elements
    when 6
      data_offset = ix * @element_size
      element_class.new(CapnProto::Reference::EMPTY, 0, @data.offset_position(data_offset), @element_size)
