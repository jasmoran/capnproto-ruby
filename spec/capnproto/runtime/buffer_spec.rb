...
describe CapnProto::Buffer, nil do
  describe ".from_buffer" do
    it "returns a new instance of CapnProto::Buffer" do
      buffer = CapnProto::Buffer.from_buffer(IO::Buffer.for("hello"))
      expect(buffer).must_be_instance_of(CapnProto::Buffer)
    end
  ...
  describe ".from_string" do
    it "returns a new instance of CapnProto::Buffer" do
      buffer = CapnProto::Buffer.from_string("hello")
      expect(buffer).must_be_instance_of(CapnProto::Buffer)
    end
  ...
  describe ".from_io" do
    it "returns a new instance of CapnProto::Buffer" do
      Tempfile.create do |file|
        buffer = CapnProto::Buffer.from_io(file)
        expect(buffer).must_be_instance_of(CapnProto::Buffer)
      end
    end
  ...
  describe "#read_integer" do
    it "reads a U8 integer from the buffer" do
      buffer = CapnProto::Buffer.from_string("\xF7\xF6\xF5\xF4\xF3\xF2\xF1\xF0")
  ...
  describe "#read_float" do
    it "reads a float from the buffer" do
      ...
      buffer = CapnProto::Buffer.from_string(data)
    ...
    it "raises an error if the offset is outside the buffer" do
      expect { CapnProto::Buffer::EMPTY.read_float(10, 64) }
        .must_raise(ArgumentError, "Type extends beyond end of buffer!")
    end

    it "raises an error if the number of bits is invalid" do
      expect { CapnProto::Buffer::EMPTY.read_float(0, 7) }
        .must_raise(ArgumentError, "Invalid type name!")
    end
  end
  ...
  describe "#dereference_pointer" do
    it "returns the reference if it is a null pointer" do
      buffer = CapnProto::Buffer.from_string("\x00\x81\xF2")
