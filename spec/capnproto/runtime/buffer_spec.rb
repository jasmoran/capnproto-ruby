...
RSpec.describe CapnProto::Buffer do
  ...
  describe "#dereference_pointer" do
    ...
    let(:struct_pointer) { CapnProto::Reference.new(buffer, 0, 1) }
    let(:list_pointer) { CapnProto::Reference.new(buffer, 1, 1) }
    let(:far_pointer) { CapnProto::Reference.new(buffer, 2, 1) }
