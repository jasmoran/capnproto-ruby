...
describe Capnp::IOBuffer, nil do
  describe ".new" do
    it "returns a new instance of Capnp::IOBuffer" do
      buffer = Capnp::IOBuffer.new(IO::Buffer.for("hello"))
      expect(buffer).must_be_instance_of(Capnp::IOBuffer)
    end
  end
  ...
  describe "#read_integer" do
    it "reads a U8 integer from the buffer" do
      buffer = Capnp::IOBuffer.new(IO::Buffer.for("\xF7\xF6\xF5\xF4\xF3\xF2\xF1\xF0"))
  ...
  describe "#read_float" do
    it "reads a float from the buffer" do
      ...
      buffer = Capnp::IOBuffer.new(IO::Buffer.for(data))
    ...
    it "raises an error if the offset is outside the buffer" do
      buffer = Capnp::IOBuffer.new(IO::Buffer.for(""))
